...
import { UserTypeService } from '../usertype/user-type.service';
...
export class AuthService {
    ...
    private setUserAuthenticated(userData: any): void {
        ...
        this.userDataService.getByUserID(userData.sub).pipe(
            switchMap(userDB =>
                this.userTypeService.getById(userDB.userTypeId).pipe(
                    catchError(err2 => {
                        console.error('Error en userTypeService', err2);
                        return of(null);
                    })
                )
            ),
            catchError(err1 => {
                console.error('Error en userDataService', err1);
                return of(null);
            }),
            finalize(() => {
            })
    ...
    hasRole(role: string): boolean {
        return this.authStateSubject.value.user?.userTypeName === role || role === '';
    }
}